...
class LJU_TableViewController: UITableViewController {
    ...
    var filtered_data:[LJU_DBModel] = []
    var todoTitle:[LJU_DBModel] = []
    
    override func viewDidLoad() {
        super.viewDidLoad()
        // --- 상단 edit버튼 활성화
        self.navigationItem.leftBarButtonItem = self.editButtonItem
        // 키보드 숨기기 objc calss전체가 TableView로 잡혀있어서 기존 터치빈으로안됨....
    ...
    override func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        return filtered_data.isEmpty ? todolist_data.count : filtered_data.count
        // todolist_data의 index만큼
    }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if (filtered_data.isEmpty){
            ...
            cell.cell_title.text = filtered_data[indexPath.row].title
            cell.cell_todo.text = filtered_data[indexPath.row].todo
            
            // preivateStatus에 따른 자물쇠 onOff
            if filtered_data[indexPath.row].privateStatus{
                cell.cell_lock.isHidden = false
            }else{
                cell.cell_lock.isHidden = true
            }
            ...
            // todo완료&미완료 에 따른 backgroundColor 및 줄긋기
            if filtered_data[indexPath.row].todoStatus{
            ...
            // cellImage 호출
            let url = URL(string: filtered_data[indexPath.row].image)
...
extension LJU_TableViewController: UISearchBarDelegate{
    ...
    func searchBar(_ searchBar: UISearchBar, textDidChange searchText: String) {
        if searchText.isEmpty {
            ...
            filtered_data = todolist_data.filter { $0.title.lowercased().contains(searchText.lowercased()) }
                }
